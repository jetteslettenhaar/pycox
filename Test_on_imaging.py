...
class SurvivalDataset(Dataset):
    ...
    def __init__(self, is_train, threshold_value=8760, sampling=None):
        ...
        '''
        self.h5_file = 'my_models/clinical_model_all_RFS_AGE_NOIMG.h5'  # Default path to .h5 file
        # loads data
...
class SurvivalDataset2(Dataset):
    ''' The dataset class performs loading data from .h5 file. '''
    def __init__(self, is_train, threshold_value=8760, sampling=None):
        ''' Loading data from .h5 file based on (is_train).
        ...
        :param sampling: (str) sampling strategy: "upsampling" or "downsampling"
        '''
        self.h5_file = 'my_models/clinical_model_RFS_AGE.h5'  # Default path to .h5 file
        # loads data
        self.X, self.e, self.y = self._read_h5_file(is_train)
        # Remove NaN values
        self._drop_na()
...
if __name__ == "__main__":
    ...
    train_dataset_2 = SurvivalDataset2(is_train=True)
    test_dataset_2 = SurvivalDataset2(is_train=False)
    combined_dataset_2 = torch.utils.data.ConcatDataset([train_dataset, test_dataset])
    print(combined_dataset_2)

    # Create custom dataloaders
    train_dataloader = DataLoader(combined_dataset, batch_size=len(combined_dataset), shuffle=True)
    test_dataloader = DataLoader(combined_dataset_2, batch_size=len(combined_dataset_2))
